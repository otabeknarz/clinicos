...
export function AboutPage() {
  ...
  useEffect(() => {
    ...
    document.title = 'ClinicOS — Klinikangizga tartib. Sizga xotirjamlik.'
    return () => {
...
/**
 * HARAKAT QATLAMI — prototipdagi `about-v3.js` ning ko'chirmasi.
 *
 * - hero matni 40 ms ketma-ketlik bilan paydo bo'ladi;
 * - bo'limlar ko'rinishga kelganda BIR MARTA kiradi;
 * - haftalik tushum chizig'i 650 ms da chiziladi;
 * - rol va bosqich almashganda mazmun yumshoq almashadi;
 * - cheksiz takrorlanadigan harakat yo'q.
 *
...
function useStoryMotion(rootRef: RefObject<HTMLElement | null>) {
  useEffect(() => {
    ...
    function enter(element: Element | null | undefined, delay = 0, distance = 18) {
      play(
        element,
        [
    ...
    if (hero) {
      Array.from(hero.children)
        ...
        .forEach((element, index) => enter(element, index * 40, 12))
    }
    ...
    root
      ...
      .forEach((element, index) => {
        if (element.matches('.v3-card,.v3-extra')) {
          element.dataset.motionDelay = String((index % 2) * 45)
        }
